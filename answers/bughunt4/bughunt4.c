 ...
 *
 * Corrected reference. Build with the answer's algo.c.
 */
...
static bool sample_cb(struct repeating_timer *t)
{
    ...
    int32_t delta = (int32_t)TARGET_MV - (int32_t)mv;
    ...
    printf("A raw=%4u filt=%4u  %4u mV (%.3f V)   B raw=%4u filt=%4u   duty=%u/%u\n",
           raw_a, (unsigned)filt_a, mv, (double)v, raw_b, (unsigned)filt_b, duty, wrap);
...
int main(void)
{
    ...
    /* Keep the sample logging out of an interrupt. The 25 ms sample interval
     * still aliases the square wave; this is not a precision DC controller. */
    while (true) {
        sample_cb(NULL);
        sleep_ms(SAMPLE_MS);
    }
